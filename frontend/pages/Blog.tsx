...
const Blog: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const schema = {
      ...
      '@type': 'Blog',
      name: `${siteConfig.name} - ${siteConfig.blog.title}`,
  ...
  return (
    <Container size="md" className="py-16">
      <div className="mb-20">
        <h1 className="text-4xl font-serif font-bold mb-4 dark:text-white">{siteConfig.blog.title}</h1>
        <p className="text-gray-700 dark:text-zinc-300 text-lg">{siteConfig.blog.description}</p>
      </div>
          ...
          <article key={post.id} className="group relative">
            <header className="mb-3">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-gray-400 dark:text-zinc-600 text-xs" aria-hidden="true">|</span>
            ...
            <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-widest">
              <span className="text-gray-600 dark:text-zinc-400">{post.readTime}</span>
              <Link to={`/blog/${post.id}`} className="inline-flex items-center gap-2 group-hover:gap-3 transition-all text-black hover:text-gray-700 dark:text-white dark:hover:text-zinc-300 focus:outline-none focus:underline">
                Read Article
            ...
            <div className="mt-16 border-b border-gray-100 dark:border-zinc-900" aria-hidden="true"></div>
          </article>
        ))}
      </div>
